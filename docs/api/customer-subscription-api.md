...
> **PROMPT**: Implement subscription plan purchase for the customer app. User browses active plans, selects a plan, completes payment (mock), and receives vouchers instantly. Show subscription status, remaining vouchers, expiry dates. Allow viewing voucher history and checking voucher eligibility before ordering. Implement subscription cancellation with refund eligibility display.
